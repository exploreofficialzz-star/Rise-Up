...
// v3.0 — Intent-Aware Smart Workflow Engine
//
// What changed from v2.0:
//   • Intent detection BEFORE any API call — 8 intent types detected from
//     freetext. The screen no longer blindly runs workflow research on every
//     query. It detects what the user actually needs first.
//   • Smart confirmation step — shows detected intent, lets user confirm or
//     switch to full workflow. High confidence = cleaner UX.
//   • Contact/job/sourcing/learn queries → routed to APEX quick agent
//   • Marketplace queries → routed directly to /marketplace
//   • Workflow intents → existing research + create flow
//   • Fully redesigned UI — natural, conversational, no raw i18n keys
//   • Better loading animation (pulsing brand card, intent-aware messages)
//   • Advanced settings collapsed by default — cleaner first impression
...
enum TaskIntent {
  incomeGoal,        // "I want to earn from YouTube" → workflow
  contactSearch,     // "find contacts of sellers" → APEX agent
  jobSearch,         // "freelance jobs in Lagos" → APEX job finder
  productSourcing,   // "where to buy cheap phones" → marketplace + APEX
  learnSkill,        // "how to learn trading" → APEX quick answer
  businessResearch,  // "how to start dropshipping" → workflow
  quickAnswer,       // general question → APEX quick
  marketplacePost,   // "I want to sell my laptop" → marketplace
}
...
IntentResult detectIntent(String text) {
  ...
  if (has(['contact', 'phone number', 'whatsapp number', 'number of',
    'give me a contact', 'contacts of', 'find someone', 'who sells',
    'sellers in', 'vendors in', 'suppliers in', 'address of'])) {
    return IntentResult(intent: TaskIntent.contactSearch,
  ...
  if (has(['where to buy', 'how to buy', 'buy cheap', 'affordable',
    'source for', 'import from', 'wholesale', 'supplier',
    'looking to buy', 'need to buy', 'want to buy', 'price of'])) {
    return IntentResult(intent: TaskIntent.productSourcing,
      label: 'Product Sourcing',
      description: "I'll find suppliers, prices, and sourcing strategies.",
      emoji: '🛒', color: const Color(0xFF6C5CE7), confidence: 0.88);
  }
  if (has(['find job', 'get job', 'job in', 'jobs for', 'freelance job',
    'remote job', 'hiring', 'vacancy', 'employment', 'work as',
    'looking for work', 'apply for', 'job opportunities'])) {
    return IntentResult(intent: TaskIntent.jobSearch,
      label: 'Job / Freelance Search',
  ...
  if (has(['i want to sell', 'i am selling', 'i sell', 'selling my',
    'sell my', 'list my', 'post my', 'want to sell', 'help me sell'])) {
    return IntentResult(intent: TaskIntent.marketplacePost,
  ...
  if (has(['how to learn', 'learn to', 'learn how to', 'teach me',
    'course for', 'tutorial on', 'beginner guide', 'explain how', 'what is'])) {
    return IntentResult(intent: TaskIntent.learnSkill,
  ...
  if (has(['how to start', 'how to build', 'how to create', 'how to launch',
    'business idea', 'side hustle', 'start a business', 'open a',
    'set up a', 'business plan'])) {
    return IntentResult(intent: TaskIntent.businessResearch,
  ...
  if (has(['i want to earn', 'make money from', 'income from',
    'i want to start earning', 'my goal is to earn', 'passive income',
    'per month', 'monthly income', 'earn \$', 'make \$'])) {
    return IntentResult(intent: TaskIntent.incomeGoal,
      label: 'Income Goal',
...
class WorkflowResearchNotifier extends StateNotifier<WorkflowResearchState> {
  ...
  // Step 1: detect intent and show confirmation
  void analyzeInput() {
  ...
  // Step 2: execute based on detected intent
  ...
  // Force workflow regardless of detected intent
  void forceWorkflow() {
  ...
  Future<void> _runQuickAgent(BuildContext context) async {
    ...
    try {
      ...
      state = state.copyWith(
        agentOutput: result['output']?.toString() ?? result.toString(),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        // Build workflow instead (for non-workflow intents)
...
// ═══════════════════════════════════════════════════════════════════
// PHASE 4a: AGENT OUTPUT (non-workflow results)
// ═══════════════════════════════════════════════════════════════════

class _AgentOutputPhase extends ConsumerWidget {
  final bool isDark;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Text(intent.emoji, style: const TextStyle(fontSize: 13)), const SizedBox(width: 6),
              Text(intent.label, style: TextStyle(color: intent.color, fontSize: 12, fontWeight: FontWeight.w700)),
            ]),
          ),
          const Spacer(),
          IconButton(
            icon: Icon(Iconsax.copy, color: sub, size: 18),
            onPressed: () {
              Clipboard.setData(ClipboardData(text: state.agentOutput));
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text('Copied to clipboard ✅'), backgroundColor: AppColors.success, duration: Duration(seconds: 2)));
            },
          ),
        ]),
        const SizedBox(height: 10),
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: intent.color.withOpacity(0.2)),
            boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 12, offset: const Offset(0, 4))],
          ),
          child: SelectableText(state.agentOutput,
              style: TextStyle(fontSize: 14, color: text, height: 1.7)),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Want a full income plan?', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: AppColors.primary)),
            const SizedBox(height: 4),
            Text('Build a step-by-step workflow with AI to execute this properly.', style: TextStyle(fontSize: 12, color: sub)),
            const SizedBox(height: 10),
            GestureDetector(
        ...
        SizedBox(width: double.infinity,
          child: TextButton.icon(
            onPressed: notifier.goBack,
            icon: Icon(Iconsax.arrow_left, color: sub, size: 14),
            label: Text('Ask something else', style: TextStyle(color: sub, fontSize: 13)),
          )),
      ]),
    );
  }
