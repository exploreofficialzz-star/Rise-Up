...
class _IncomeMemoryScreenState extends State<IncomeMemoryScreen>
  ...
  /// Guard so the interstitial only fires once per screen session.
  /// If the user dismisses insights and re-taps, we skip the ad.
  bool _adAlreadyServed = false;
  ...
  // ─────────────────────────────────────────────────────────
  // AD-GATED INSIGHTS UNLOCK
  // ─────────────────────────────────────────────────────────

  /// Entry point for the "Unlock My Insights" button.
  ///
  /// Free users  → forced interstitial (once per session) → load insights.
  /// Premium users → load insights directly.
  Future<void> _onUnlockInsightsTap() async {
    if (_loadingInsights) return;

    if (!adManager.isPremium && !_adAlreadyServed) {
      _adAlreadyServed = true;
      // Force show interstitial — bypasses frequency/cooldown caps.
      // adManager handles "no ad loaded" gracefully (no-op).
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('⚠️ Could not load insights. Check your connection.'),
  ...
  Future<void> _logEvent(String type, String title, double amount) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to record event. Please try again.'),
            backgroundColor: AppColors.error,
          ),
        );
      }
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'What happened? (e.g. Completed logo for client)',
                    hintStyle: TextStyle(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('✅ Event recorded to memory!'),
      ...
      body: _loading
          ...
          : Column(
              children: [
                ...
                // ── Sticky inline banner for free users ──────────────
                // Renders below tab content, above the app shell nav bar.
                // getStickyBanner() returns SizedBox.shrink() for premium.
                adManager.getStickyBanner(context),
              ],
            ),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                  'Complete tasks, log earnings, and win clients — your income DNA builds automatically.',
                  textAlign: TextAlign.center,
                  style:
                      TextStyle(color: sub, fontSize: 13, height: 1.5)),
              const SizedBox(height: 24),
              GestureDetector(
                onTap: _showLogEventSheet,
                child: Container(
                  padding: const EdgeInsets.symmetric(
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Container(
        ...
        child: Column(children: [
          Row(children: [
            _dnaStatW('Total Earned',
                '\$${(p['total_earned_usd'] ?? 0).toString()}',
                Colors.white),
            _dnaStatW(
                'Events', '${p['total_events'] ?? 0}', Colors.white),
            _dnaStatW('Completion',
                '${p['completion_rate_pct'] ?? 0}%', Colors.white),
          ]),
          const SizedBox(height: 14),
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                        'Your Best Platform: ${p['best_platform'] ?? 'Not enough data yet'}',
                        style: const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.w700,
                            fontSize: 13)),
                    Text(
                        'Your Strongest Skill: ${p['best_skill'] ?? 'Keep logging to discover'}',
                        style: const TextStyle(
      ...
      if ((p['platform_breakdown'] as Map? ?? {}).isNotEmpty) ...[
        ...
        ...(p['platform_breakdown'] as Map).entries.take(5).map((e) {
          final max = (p['platform_breakdown'] as Map)
              .values
              .reduce((a, b) => a > b ? a : b);
      ...
      if ((p['skill_breakdown'] as Map? ?? {}).isNotEmpty) ...[
        ...
        Wrap(
          ...
          children: (p['skill_breakdown'] as Map).entries.take(6).map((e) {
            ...
            final idx = (p['skill_breakdown'] as Map)
                    .keys
                    .toList()
                    .indexOf(e.key) %
                colors.length;
      ...
      if ((p['recent_events'] as List? ?? []).isNotEmpty) ...[
        ...
        ...(p['recent_events'] as List).take(5).map((e) {
          ...
          return Container(
            ...
            child: Row(children: [
              Text(_eventEmoji(ev['event_type']?.toString() ?? ''),
  ...
  Widget _buildInsights(bool isDark, Color text, Color sub) {
    final rawInsights = _insights['insights'];

    // ── Empty state: no insights loaded yet ──────────────────
    if (rawInsights == null || _insights.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                  'Your AI analyses your income patterns and gives personalized intelligence.',
                  textAlign: TextAlign.center,
                  style:
                      TextStyle(color: sub, fontSize: 13, height: 1.5)),
              // ── Ad badge for free users ─────────────────────
    ...
    // ── FIX: backend returns {"insights": "string message", "ready": false}
    //    when the user has fewer than 3 events.
    //    Casting a String to Map throws → crash. Handle both cases.
          ...
          child: Column(
            ...
            children: [
              ...
              Text(rawInsights,
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      color: text, fontSize: 14, height: 1.6)),
              const SizedBox(height: 24),
              GestureDetector(
                onTap: _showLogEventSheet,
                child: Container(
    ...
    // ── Loaded state: insights is a Map ──────────────────────
    final insightsMap = (rawInsights as Map?) ?? {};
    ...
    final visibleCards = insightCards
        .where((c) => c.$3 != null && c.$3.toString().isNotEmpty)
        .toList();
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('AI analyzed your income history',
                    style: TextStyle(
                        color: Colors.white,
                        fontWeight: FontWeight.w700,
                        fontSize: 13)),
                Text(
                    '${_insights['events_analyzed'] ?? 0} events processed',
                    style: const TextStyle(
      ...
      ...visibleCards.asMap().entries.map((entry) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: c.$4.withOpacity(isDark ? 0.05 : 0.07),
                blurRadius: 8,
      ...
      // Inline banner ad between insights and bottom padding (free users)
      if (!adManager.isPremium) ...[
        const SizedBox(height: 8),
  ...
  Widget _buildPatterns(bool isDark, Color text, Color sub) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              Text(
                  'Log events over several days to see when you earn most.',
                  textAlign: TextAlign.center,
                  style:
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      ...patterns.asMap().entries.map((entry) {
        ...
        final maxEarned = patterns
            .map((d) => (d as Map)['earned'] ?? 0)
            .reduce((a, b) => a > b ? a : b);
        final pct = maxEarned > 0
            ? ((p['earned'] ?? 0) / maxEarned).clamp(0.0, 1.0).toDouble()
            : 0.0;
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                ...
                const Spacer(),
                Text('\$${(p['earned'] ?? 0).toStringAsFixed(0)}',
                    style: TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w800,
                        color: isBest
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation(
                      isBest ? AppColors.gold : AppColors.primary),
                ),
              ),
              const SizedBox(height: 4),
              Text(
                  '${p['completed'] ?? 0} tasks · ${p['count'] ?? 0} events',
                  style: TextStyle(fontSize: 11, color: sub)),
            ],
          ),
  ...
  Widget _dnaStatW(String label, String value, Color color) =>
      Expanded(
        child: Column(children: [
          Text(value,
              style: TextStyle(
