...
//
// FIXED:
//  1. Video green frame — setVolume(0)→play()→500ms→pause()→seekTo(0)
//  2. Delete crash — Future.microtask on onPostDeleted callback
//  3. Ask RiseUp AI — real error extracted from ApiException.statusCode
//  4. Link post — placeholder content normalised; linkUrl shown always
//  5. AdMob — FeedAdCard wraps real BannerAd with promo card fallback
//  6. Status video — same green-frame fix applied
...
class PostModel {
  ...
  factory PostModel.fromApi(Map<String, dynamic> d) {
    ...
    final String t;
    if (diff.inMinutes < 60)    t = '${diff.inMinutes}m ago';
    else if (diff.inHours < 24) t = '${diff.inHours}h ago';
    else                        t = '${diff.inDays}d ago';
    ...
    final userId    = d['user_id']?.toString().isNotEmpty == true
        ? d['user_id'].toString() : profile['id']?.toString() ?? '';

    // FIX: strip link-only placeholder content
    String raw = d['content']?.toString() ?? '';
    if (raw == '🔗 Link post' || raw == 'Link post') raw = '';

    return PostModel(
      id: d['id']?.toString() ?? '', name: name,
      username: '@${name.toLowerCase().replaceAll(' ', '')}',
      time: t, avatar: stageInfo['emoji'] as String,
      avatarUrl: profile['avatar_url']?.toString() ?? '',
      tag: d['tag']?.toString() ?? '💰 Wealth', content: raw,
...
class _Sh extends StatelessWidget {
  ...
  final double? w; final double h, r; final bool circle;
  @override
...
class _PostCardSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext ctx) {
    ...
    return Container(color: isDark ? AppColors.bgCard : Colors.white,
      padding: const EdgeInsets.fromLTRB(16, 14, 16, 14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const _Sh(w: 44, h: 44, circle: true), const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _Sh(w: w * .35, h: 13), const SizedBox(height: 5), _Sh(w: w * .25, h: 11),
          ])), const _Sh(w: 60, h: 26, r: 13)]),
        const SizedBox(height: 14),
        const _Sh(h: 13), const SizedBox(height: 6),
        const _Sh(h: 13), const SizedBox(height: 6),
        _Sh(w: w * .55, h: 13), const SizedBox(height: 12),
        AspectRatio(aspectRatio: 16/9, child: _Sh(h: double.infinity, r: 12)),
        const SizedBox(height: 14),
        Row(children: const [_Sh(w: 55, h: 18, r: 9), SizedBox(width: 18),
          _Sh(w: 55, h: 18, r: 9), SizedBox(width: 18),
          _Sh(w: 55, h: 18, r: 9), Spacer(), _Sh(w: 22, h: 22, r: 11)]),
        const SizedBox(height: 12),
        Row(children: const [Expanded(child: _Sh(h: 36, r: 10)), SizedBox(width: 8),
...
class _StoriesSkel extends StatelessWidget {
  ...
  Widget build(BuildContext ctx) => Container(
    color: isDark ? AppColors.bgCard : Colors.white, height: 92,
    child: ListView.builder(scrollDirection: Axis.horizontal,
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
      itemCount: 5,
      itemBuilder: (_, __) => const Padding(padding: EdgeInsets.only(right: 14),
        child: Column(children: [_Sh(w: 58, h: 58, circle: true), SizedBox(height: 5),
          _Sh(w: 42, h: 10, r: 5)]))));
}
...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _restoreCache() async {
    ...
    try {
      ...
      if (r != null) {
        final posts = (jsonDecode(r) as List)
            .map((x) => PostModel.fromApi(Map<String, dynamic>.from(x as Map))).toList();
    ...
    try {
      ...
      if (r != null) {
        ...
        if (sv['date'] == today && mounted) setState(() {
  ...
  Future<void> _refreshAll() => Future.wait([_loadProfile(), _loadStatus(), _loadFeed('for_you', refresh: true)]);
  ...
  Future<void> _loadStatus() async {
    ...
    } catch (_) { if (mounted) setState(() => _statusLoaded = true); }
  }
  ...
  Future<void> _loadFeed(String tab, {bool refresh = false}) async {
    ...
    try {
      ...
      if (mounted) setState(() {
        ...
        if (refresh) _feeds[tab] = posts; else _feeds[tab] = [..._feeds[tab]!, ...posts];
        _offsets[tab] = (_offsets[tab] ?? 0) + posts.length;
        _hasMore[tab] = posts.length == 20;
        _loading[tab] = false;
      });
    } catch (_) { if (mounted) setState(() => _loading[tab] = false); }
  }
  ...
  Future<void> _handleFollow(String uid) async {
    ...
    } catch (_) { if (mounted) setState(() { _follows[uid] = prev; _syncFw(uid, prev); }); }
  }

  void _syncFw(String uid, bool v) {
    for (final t in _tabs) for (final p in _feeds[t]!) if (p.userId == uid) p.isFollowing = v;
  }
  ...
  void _handleShare(PostModel post) {
    ...
    showModalBottomSheet(context: context,
      ...
      builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ListTile(leading: Container(width: 40, height: 40,
  ...
  Future<void> _handleAI(PostModel post, {required bool isPrivate}) async {
    ...
    if (_aiUsed < _freeLimit) { setState(() => _aiUsed++); await _saveQuota(); await _execAI(post, priv: isPrivate); return; }
    if (_adLocked) { _showLockout(); return; }
    if (_adsWatched >= _maxAds) { setState(() => _adLockout = DateTime.now().add(_lockDur)); await _saveQuota(); _showLockout(); return; }
    final ok = await _showAdPrompt();
    if (!ok || !mounted) return;
    await adService.showRewardedAd(
      featureKey: 'post_ai',
      onRewarded: () async { setState(() { _aiUsed = 0; _adsWatched++; }); await _saveQuota(); if (mounted) await _execAI(post, priv: isPrivate); },
      onDismissed: () { if (mounted) _snack('Watch the full ad to unlock AI.', AppColors.error); },
    );
  }
  ...
  // FIX v6.0: proper error extraction
  Future<void> _postAIComment(PostModel post) async {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(children: const [SizedBox(width: 18, height: 18,
          child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)),
        SizedBox(width: 12), Text('RiseUp AI is thinking…')]),
      backgroundColor: AppColors.primary, duration: const Duration(seconds: 120)));
    ...
    try {
      ...
      if (e.statusCode == 429) err = 'AI rate limit reached. Please wait a moment.';
      else if (e.statusCode != null && e.statusCode! >= 500) err = 'AI temporarily unavailable.';
      else err = e.message.isNotEmpty ? e.message : 'AI request failed.';
    } catch (e) {
      final s = e.toString();
      if (s.contains('Timeout') || s.contains('Socket')) err = 'Connection timed out. Check your network.';
      else { err = s.replaceAll('Exception: ', ''); if (err.length > 100) err = '${err.substring(0, 100)}…'; }
    }

    ScaffoldMessenger.of(context).hideCurrentSnackBar();
    if (!mounted) return;
    _snack(err.isEmpty ? 'AI request failed. Please try again.' : err, AppColors.error,
    ...
    return await showDialog<bool>(context: context,
        builder: (_) => AlertDialog(
          ...
          title: Text('Watch a short ad?', style: TextStyle(fontWeight: FontWeight.w700,
              fontSize: 18, color: dark ? Colors.white : Colors.black87)),
          content: Text('You\'ve used your $_freeLimit free responses today.\n\n'
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(icon: Icon(Icons.menu_rounded, color: ico, size: 24),
            onPressed: () { HapticFeedback.lightImpact(); _sk.currentState?.openDrawer(); }),
        title: ShaderMask(
          shaderCallback: (b) => const LinearGradient(
              colors: [Color(0xFFFF6B00), Color(0xFFFFD700), Color(0xFF6C5CE7)],
              stops: [0.0, 0.4, 1.0]).createShader(b),
          child: const Text('RiseUp', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900,
              color: Colors.white, letterSpacing: -0.5))),
        centerTitle: true,
        actions: [
          if (!_isPremium) Container(
          ...
          IconButton(icon: Icon(Iconsax.search_normal, color: ico, size: 20),
              onPressed: () { HapticFeedback.lightImpact(); context.go('/explore'); }),
          IconButton(icon: Icon(Iconsax.notification, color: ico, size: 20),
      ...
      body: Column(children: [
        ...
        Container(color: card, child: Column(children: [
          AnimatedSwitcher(duration: const Duration(milliseconds: 300),
            child: !_statusLoaded
                ...
                : SizedBox(key: const ValueKey('r'), height: 92,
                    child: ListView.builder(scrollDirection: Axis.horizontal,
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                      itemCount: _statusUsers.length + 1,
                      itemBuilder: (_, i) {
                        if (i == 0) return _StoryAdd(isDark: dark,
        ...
        // Tabs
        Container(color: card, child: Column(children: [
          TabBar(controller: _tab, labelColor: AppColors.primary, unselectedLabelColor: sub,
        ...
        // Feed
        Expanded(child: TabBarView(controller: _tab,
          children: _tabs.map((tab) => _FeedTab(
            key: PageStorageKey(tab), tab: tab,
            posts: _feeds[tab]!, isLoading: _loading[tab] == true,
            ...
            onFollow: _handleFollow, onShare: _handleShare,
            // FIX v6.0: microtask prevents setState-during-dispose crash
            ...
            onLike: (p) async {
              ...
              } catch (_) { if (mounted) setState(() { p.isLiked = !p.isLiked; p.likes += p.isLiked ? 1 : -1; }); }
            },
            onSave: (p) async {
              HapticFeedback.mediumImpact();
              ...
              } catch (_) { if (mounted) setState(() => p.isSaved = !p.isSaved); }
            },
            onComment: (p) => context.push('/comments/${p.id}'
...
// ─────────────────────────────────────────────────────────────────────────────
// Feed Tab — AutomaticKeepAliveClientMixin
// ─────────────────────────────────────────────────────────────────────────────
class _FeedTab extends StatefulWidget {
  final String tab; final List<PostModel> posts;
  final bool isLoading, hasMore, isDark, isPremium, needsAd;
  final Color cardColor, borderColor, textColor, subColor;
  final int aiRemaining; final String currentUserId;
  ...
  const _FeedTab({
    super.key, required this.tab, required this.posts,
    ...
    required this.isPremium, required this.aiRemaining, required this.needsAd,
    ...
    required this.onLike, required this.onSave, required this.onComment,
    required this.onShare, required this.onFollow, required this.onPostDeleted,
  });
...
class _FeedTabState extends State<_FeedTab> with AutomaticKeepAliveClientMixin {
  ...
  void _onScroll() {
    ...
    if (pos.pixels >= pos.maxScrollExtent * 0.7 && !_pf && !widget.isLoading && widget.hasMore) {
      _pf = true; widget.onLoadMore();
      Future.delayed(const Duration(seconds: 3), () { if (mounted) _pf = false; });
    }
  }
  ...
  Widget build(BuildContext ctx) {
    super.build(ctx);
    final posts = widget.posts; final border = widget.borderColor;

    if (widget.isLoading && posts.isEmpty) return ListView.separated(
    ...
    if (posts.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      GestureDetector(onTap: widget.onRefresh,
    ...
    return RefreshIndicator(onRefresh: () async => widget.onRefresh(), color: AppColors.primary,
      child: ListView.separated(controller: _sc, cacheExtent: 1500, padding: EdgeInsets.zero,
        itemCount: total,
        separatorBuilder: (_, __) => Divider(height: 8, thickness: 8, color: border),
        itemBuilder: (_, i) {
          if (i == total - 1) {
            if (widget.isLoading) return _PostCardSkeleton(isDark: widget.isDark);
            if (!widget.hasMore) return Padding(padding: const EdgeInsets.all(20),
                child: Center(child: Text("You're all caught up ✓",
                    style: TextStyle(color: widget.subColor, fontSize: 13))));
            return const SizedBox(height: 40);
          }
          // FIX v6.0: FeedAdCard now wraps real AdMob BannerAd with promo fallback
          if (adManager.shouldShowFeedAd(i)) return FeedAdCard(
          ...
          return PostCard(
            ...
            needsAd: widget.needsAd, isFollowing: fol,
...
class PostCard extends StatefulWidget {
  ...
  const PostCard({
    super.key, required this.post, required this.isDark,
    ...
    required this.isFollowing, this.needsAd = false, this.currentUserId = '',
  });
...
class _PostCardState extends State<PostCard> {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          TextField(controller: ctrl, maxLines: 5, style: TextStyle(color: widget.isDark ? Colors.white : Colors.black87),
            decoration: InputDecoration(hintText: "What's on your mind?", hintStyle: TextStyle(color: widget.subColor),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            ...
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              try { await api.updatePost(widget.post.id, content: t); if (mounted) setState(() => widget.post.content = t); }
              catch (_) {}
            },
    ...
    showModalBottomSheet(context: context,
      ...
      builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(width: 36, height: 4, margin: const EdgeInsets.only(top: 12, bottom: 8),
            decoration: BoxDecoration(color: Colors.grey.withOpacity(0.3), borderRadius: BorderRadius.circular(2))),
        if (_isOwn) ...[
          ListTile(leading: const Icon(Iconsax.edit, color: AppColors.primary),
          ...
          ListTile(leading: const Icon(Iconsax.trash, color: AppColors.error),
        ...
        ListTile(leading: Icon(Iconsax.copy, color: widget.isDark ? Colors.white70 : Colors.black54),
        ...
        ListTile(leading: Icon(Iconsax.share, color: widget.isDark ? Colors.white70 : Colors.black54),
  ...
  @override
  Widget build(BuildContext ctx) {
    final p = widget.post; final sw = MediaQuery.of(ctx).size.width;
    return Container(color: widget.cardColor,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 0),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              GestureDetector(onTap: () => ctx.push('/user-profile/${p.userId}'),
                  child: _Avatar(url: p.avatarUrl, fallback: p.avatar, size: 44)),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Flexible(child: GestureDetector(onTap: () => ctx.push('/user-profile/${p.userId}'),
                  ...
                  if (p.isPremiumPost) ...[const SizedBox(width: 4),
                ...
                Text('${p.username} · ${p.time}', style: TextStyle(fontSize: 12, color: widget.subColor),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
              ])),
              const SizedBox(width: 6),
              if (p.userId.isNotEmpty && !_isOwn)
                GestureDetector(onTap: () => widget.onFollow(p.userId),
              ...
              const SizedBox(width: 4),
              Flexible(child: Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3),
            ...
            const SizedBox(height: 12),
            // Content
            if (p.content.isNotEmpty) _HTag(text: p.content,
                textColor: widget.isDark ? const Color(0xFFE8E8F0) : Colors.black87),
            // Media
            if (p.mediaUrl != null && p.mediaUrl!.isNotEmpty) ...[
              const SizedBox(height: 10),
              _PostMedia(url: p.mediaUrl!, mediaType: p.mediaType ?? 'image', isDark: widget.isDark, sw: sw),
            ],
            // Link card — FIX: shown for link-only posts (content may be empty)
            if (p.linkUrl != null && p.linkUrl!.isNotEmpty) ...[
              const SizedBox(height: 10),
              _LinkCard(url: p.linkUrl!, title: p.linkTitle,
                  isDark: widget.isDark, sub: widget.subColor, txt: widget.textColor),
            ],
            SizedBox(height: (p.content.isNotEmpty || p.mediaUrl != null || p.linkUrl != null) ? 14 : 4),
            // Actions
            Row(children: [
              _ActBtn(icon: p.isLiked ? Icons.favorite_rounded : Icons.favorite_border_rounded,
                  label: _fmt(p.likes), color: p.isLiked ? Colors.red : widget.subColor,
                  onTap: () => widget.onLike(p)),
              const SizedBox(width: 18),
              _ActBtn(icon: Iconsax.message, label: _fmt(p.comments), color: widget.subColor, onTap: () => widget.onComment(p)),
              const SizedBox(width: 18),
              _ActBtn(icon: Iconsax.send_1, label: _fmt(p.shares), color: widget.subColor, onTap: () => widget.onShare(p)),
              const Spacer(),
              GestureDetector(onTap: () => widget.onSave(p),
        ...
        Container(
          ...
          child: Row(children: [
            Expanded(child: GestureDetector(onTap: () => widget.onAskAI(p),
              child: Container(padding: const EdgeInsets.symmetric(vertical: 9),
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const Flexible(child: Text('Ask RiseUp AI',
                      ...
                      maxLines: 1, overflow: TextOverflow.ellipsis)),
                  if (widget.needsAd) ...[const SizedBox(width: 4),
                    Icon(Icons.ondemand_video_rounded, size: 13, color: AppColors.primary.withOpacity(0.7))],
                ])))),
            const SizedBox(width: 8),
            Expanded(child: GestureDetector(onTap: () => widget.onPrivateChat(p),
              child: Container(padding: const EdgeInsets.symmetric(vertical: 9),
                decoration: BoxDecoration(
                ...
                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Iconsax.lock_1, color: AppColors.accent, size: 14), SizedBox(width: 6),
...
class _Avatar extends StatelessWidget {
  final String url, fallback; final double size;
  ...
  Widget build(BuildContext ctx) => Container(width: size, height: size,
    ...
    child: ClipOval(child: url.isNotEmpty
        ? CachedNetworkImage(imageUrl: url, fit: BoxFit.cover, width: size, height: size,
            placeholder: (_, __) => _fb(), errorWidget: (_, __, ___) => _fb())
        : _fb()));
  Widget _fb() => Container(color: AppColors.primary.withOpacity(0.15),
...
class _HTagState extends State<_HTag> {
  ...
  List<InlineSpan> _spans() {
    ...
    final spans = <InlineSpan>[]; final re = RegExp(r'(#\w+)'); int last = 0;
    for (final m in re.allMatches(widget.text)) {
      ...
      final tag = m.group(0)!;
      final rec = TapGestureRecognizer()..onTap = () { HapticFeedback.lightImpact(); context.push('/explore?q=${Uri.encodeComponent(tag)}'); };
      _r.add(rec);
      spans.add(TextSpan(text: tag, style: const TextStyle(color: AppColors.primary, fontSize: 14.5,
          height: 1.6, fontWeight: FontWeight.w600), recognizer: rec));
      last = m.end;
    }
...
class _PostMedia extends StatelessWidget {
  final String url, mediaType; final bool isDark; final double sw;
  const _PostMedia({required this.url, required this.mediaType, required this.isDark, required this.sw});
  @override Widget build(BuildContext ctx) => ClipRRect(borderRadius: BorderRadius.circular(12),
    child: mediaType == 'video' ? _VidThumb(url: url, isDark: isDark) : _ImgThumb(url: url, isDark: isDark));
}
...
class _ImgThumb extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => GestureDetector(
    ...
    child: AspectRatio(aspectRatio: 4/3, child: Hero(tag: 'img_$url',
        child: CachedNetworkImage(imageUrl: url, fit: BoxFit.cover, width: double.infinity,
            ...
            errorWidget: (_, __, ___) => Container(color: isDark ? Colors.grey.shade900 : Colors.grey.shade200,
                child: const Center(child: Icon(Icons.broken_image_rounded, color: Colors.white54, size: 36)))))));
}

// FIX v6.0: Green frame fix — play→500ms→pause→seek(0) before showing widget
...
class _VidThumbState extends State<_VidThumb> {
  VideoPlayerController? _c; bool _ready = false;
  ...
  Future<void> _init() async {
    try {
      ...
      await c.setVolume(0.0);   // 1. mute
      await c.play();           // 2. start decode
      await Future.delayed(const Duration(milliseconds: 500)); // 3. wait for valid frame
      if (!mounted) { c.dispose(); return; }
      await c.pause();          // 4. stop
      await c.seekTo(Duration.zero); // 5. back to start (surface is valid now)
      if (!mounted) { c.dispose(); return; }
      setState(() { _c = c; _ready = true; }); // 6. safe to show
    } catch (_) {}
  }
  ...
  Widget build(BuildContext ctx) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(ctx, MaterialPageRoute(fullscreenDialog: true, builder: (_) => _VidFull(url: widget.url))),
      child: AspectRatio(aspectRatio: ratio, child: Stack(children: [
        if (_ready && _c != null) Positioned.fill(child: VideoPlayer(_c!))
        else Positioned.fill(child: Container(color: widget.isDark ? Colors.grey.shade900 : Colors.grey.shade200,
            child: const Center(child: SizedBox(width: 28, height: 28,
                child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white38))))),
        Positioned.fill(child: Center(child: Container(width: 56, height: 56,
        ...
        Positioned(bottom: 10, right: 10, child: Container(
          ...
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.play_circle_outline_rounded, color: Colors.white, size: 12), SizedBox(width: 4),
...
class _VidFullState extends State<_VidFull> {
  VideoPlayerController? _vp; ChewieController? _ch;
  ...
  Future<void> _init() async {
    try {
      ...
      await vp.initialize();
      final ch = ChewieController(videoPlayerController: vp, autoPlay: true, looping: false,
        ...
        materialProgressColors: ChewieProgressColors(playedColor: AppColors.primary,
            handleColor: AppColors.primary, backgroundColor: Colors.grey.shade800,
  ...
  @override Widget build(BuildContext ctx) => Scaffold(
    ...
    body: Center(child: _err
        ? const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.error_outline_rounded, color: Colors.white54, size: 64), SizedBox(height: 16),
            Text('Could not load video', style: TextStyle(color: Colors.white70, fontSize: 16))])
        : _ready && _ch != null ? Chewie(controller: _ch!)
...
class _ImgView extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => Scaffold(
    ...
    appBar: AppBar(backgroundColor: Colors.black.withOpacity(0.4), elevation: 0,
        iconTheme: const IconThemeData(color: Colors.white),
        actions: [
...
class _LinkCard extends StatelessWidget {
  ...
  final bool isDark; final Color sub, txt;
  ...
  @override Widget build(BuildContext ctx) => GestureDetector(onTap: () => _open(ctx),
    child: Container(height: 68, padding: const EdgeInsets.symmetric(horizontal: 12),
...
class _ActBtn extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => GestureDetector(onTap: onTap, child: Row(children: [
    Icon(icon, color: color, size: 20), const SizedBox(width: 5),
...
class _AppDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext ctx) {
    ...
    return Drawer(backgroundColor: bg, width: MediaQuery.of(ctx).size.width * 0.82,
      child: SafeArea(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 16),
          child: Row(children: [
            GestureDetector(onTap: () { Navigator.pop(ctx); ctx.go('/profile'); },
                child: _Avatar(url: av, fallback: name.isNotEmpty ? name[0].toUpperCase() : 'U', size: 48)),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                    decoration: BoxDecoration(color: (si['color'] as Color).withOpacity(0.15), borderRadius: BorderRadius.circular(8)),
                    child: Text('${si['emoji']} ${si['label']}',
                        style: TextStyle(fontSize: 10, color: si['color'] as Color, fontWeight: FontWeight.w600))),
        ...
        if (!isPro) Padding(padding: const EdgeInsets.all(14), child: GestureDetector(
          onTap: () { Navigator.pop(ctx); ctx.push('/premium'); },
          child: Container(padding: const EdgeInsets.all(13),
            decoration: BoxDecoration(color: AppColors.primary.withOpacity(isDark ? 0.12 : 0.07),
                borderRadius: BorderRadius.circular(12),
...
class _DS extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => Padding(padding: const EdgeInsets.fromLTRB(18, 12, 18, 4),
...
class _DI extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) {
    ...
    return Material(color: Colors.transparent, child: InkWell(onTap: () { HapticFeedback.lightImpact(); onTap(); },
            ...
            Row(children: [
              ...
              if (badge != null) ...[const SizedBox(width: 6),
                Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2),
                    decoration: BoxDecoration(color: (bc ?? AppColors.primary).withOpacity(0.15), borderRadius: BorderRadius.circular(5)),
                    child: Text(badge!, style: TextStyle(fontSize: 9, color: bc ?? AppColors.primary, fontWeight: FontWeight.w700)))],
            ]),
...
class _StoryAdd extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) => Padding(padding: const EdgeInsets.only(right: 14),
    ...
    return Padding(padding: const EdgeInsets.only(right: 14),
      child: GestureDetector(onTap: onTap, child: Column(children: [
        Stack(children: [
          Container(width: 58, height: 58,
            decoration: BoxDecoration(shape: BoxShape.circle,
              gradient: unseen ? const LinearGradient(colors: [Color(0xFFFF6B00), Color(0xFF6C5CE7)],
            ...
            child: Padding(padding: const EdgeInsets.all(2.5),
              child: Container(decoration: BoxDecoration(shape: BoxShape.circle,
                ...
                child: ClipOval(child: av.isNotEmpty
                    ? CachedNetworkImage(imageUrl: av, fit: BoxFit.cover, width: 53, height: 53,
                        errorWidget: (_, __, ___) => _ini(name)) : _ini(name))))),
          if (on) Positioned(bottom: 1, right: 1, child: Container(width: 13, height: 13,
              decoration: BoxDecoration(color: AppColors.success, shape: BoxShape.circle,
                  border: Border.all(color: isDark ? Colors.black : Colors.white, width: 2)))),
        ]),
        const SizedBox(height: 5),
        SizedBox(width: 62, child: Text(name.split(' ').first, textAlign: TextAlign.center,
            maxLines: 1, overflow: TextOverflow.ellipsis,
            style: TextStyle(fontSize: 11, color: isDark ? Colors.white.withOpacity(0.6) : Colors.black54))),
      ])));
  }
  Widget _ini(String n) => Container(color: AppColors.primary.withOpacity(0.15),
...
class _StatusViewSheetState extends State<_StatusViewSheet> {
  int _idx = 0;
  @override void initState() { super.initState(); _mark(); }
  void _mark() {
  ...
  void _next() { final items = widget.user['items'] as List? ?? [];
    if (_idx < items.length - 1) { setState(() { _idx++; _mark(); }); } else { Navigator.pop(context); } }
  void _prev() { if (_idx > 0) setState(() { _idx--; _mark(); }); }
  ...
  @override Widget build(BuildContext ctx) {
    final items = (widget.user['items'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    final prof  = (widget.user['profile'] as Map?)?.cast<String, dynamic>() ?? {};
    if (items.isEmpty) return const SizedBox.shrink();
    final sz    = MediaQuery.of(ctx).size;
    final h = sz.height * 0.92; final sw = sz.width;
    final name  = prof['full_name']?.toString() ?? 'User';
    final av    = prof['avatar_url']?.toString() ?? '';
    ...
    return Container(height: h,
      decoration: BoxDecoration(color: media != null ? Colors.black : bgc,
      ...
      child: Stack(children: [
        ...
        // 2. Progress
        Positioned(top: 12, left: 12, right: 12, child: Row(
            children: List.generate(items.length, (i) => Expanded(child: Container(height: 3,
              margin: const EdgeInsets.symmetric(horizontal: 2),
              decoration: BoxDecoration(color: i <= _idx ? Colors.white : Colors.white.withOpacity(0.3),
        ...
        // 5. Link card
        if (link != null)
          Positioned(bottom: 80, left: 16, right: 16, child: GestureDetector(onTap: () => _openLink(link),
            child: Container(padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(color: Colors.black.withOpacity(0.7),
                  borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.white.withOpacity(0.3))),
              child: Row(children: [
                ...
                const SizedBox(width: 6), const Icon(Icons.open_in_new_rounded, color: Colors.white70, size: 16),
              ])))),
...
// FIX v6.0: Same green-frame fix applied to status video player
...
class _StatusVidState extends State<_StatusVid> {
  VideoPlayerController? _c; bool _ready = false, _err = false;
  @override void initState() { super.initState(); _init(); }
  Future<void> _init() async {
    try {
      final c = VideoPlayerController.networkUrl(Uri.parse(widget.url));
    ...
    return GestureDetector(
      ...
      child: Stack(alignment: Alignment.center, children: [
        SizedBox.expand(child: FittedBox(fit: BoxFit.cover,
            child: SizedBox(width: _c!.value.size.width, height: _c!.value.size.height,
                child: VideoPlayer(_c!)))),
        if (!_c!.value.isPlaying) Container(width: 56, height: 56,
            decoration: BoxDecoration(color: Colors.black.withOpacity(0.6), shape: BoxShape.circle),
            child: const Icon(Icons.play_arrow_rounded, color: Colors.white, size: 36)),
