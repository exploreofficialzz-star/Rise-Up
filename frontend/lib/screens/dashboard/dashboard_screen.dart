...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  // ── Load: cache-first then silent background refresh ─────
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        drawer: _DashboardDrawer(
          profile: profile, isPremium: isPremium, isDark: isDark,
          bgCard: bgCard, bgSurface: bgSurface,
          txtPrimary: txtPrimary, txtSecond: txtSecond,
          txtMuted: txtMuted, divider: divider,
        ),
        body: RefreshIndicator(
          ...
          child: _networkError
              ? _NetworkErrorView(
                  bg: bg, txtPrimary: txtPrimary, txtSecond: txtSecond,
              ...
              : CustomScrollView(
                  ...
                  slivers: [
                    ...
                    _DashboardAppBar(
                      ...
                      greeting: _greeting,
                      name: name,
                      avatarUrl: avatarUrl,        // ← real photo
                      stage: stage,
                      stageInfo: stageInfo,
                      isDark: isDark,
                        ...
                        child: _loading
                            ...
                            : Column(
                                ...
                                children: [
                                  ...
                                  // ── Streak chip ────────
                                  if ((_streak['current_streak'] as int? ?? 0) > 0) ...[
                                    _StreakBanner(
                                      streak: _streak,
                                      isDark: isDark,
                                      bgCard: bgCard,
...
// ─────────────────────────────────────────────────────────────
//  Facebook-style skeleton (shimmer on every section)
// ─────────────────────────────────────────────────────────────
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
              color: bgCard, borderRadius: AppRadius.xl),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _bone(h: 11, w: 160, delayMs: 0),
              const SizedBox(height: 14),
              _bone(h: 38, w: 180, r: 10, delayMs: 60),
              const SizedBox(height: 16),
              Row(children: [
                _bone(h: 22, w: 100, r: 20, delayMs: 120),
                const SizedBox(width: 8),
                _bone(h: 11, w: 130, delayMs: 140),
              ]),
            ],
          ),
        ),
        ...
        Row(children: List.generate(3, (i) => Expanded(
          child: Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  color: bgCard, borderRadius: AppRadius.lg),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  _bone(h: 20, w: 20, circle: true, delayMs: i * 80),
                  const SizedBox(height: 10),
                  _bone(h: 20, w: 40, r: 6, delayMs: 60 + i * 80),
                  const SizedBox(height: 6),
                  _bone(h: 9, w: 60, delayMs: 100 + i * 80),
                ],
              ),
            ),
          ),
        ...
        // ── Section label ──────────────────────────────────
        _bone(h: 14, w: 110, delayMs: 100),
        const SizedBox(height: 12),

        // ── Income tools grid (4 tiles × 2) ───────────────
        GridView.builder(
          itemCount: 8,
          shrinkWrap: true,
          ...
          itemBuilder: (_, i) => Container(
            ...
            decoration: BoxDecoration(
                color: bgCard, borderRadius: AppRadius.lg),
            child: Row(children: [
              _bone(h: 22, w: 22, circle: true, delayMs: i * 40),
              ...
              Expanded(
                child: Column(
                  ...
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    _bone(h: 11, delayMs: 60 + i * 40),
                    const SizedBox(height: 5),
                    _bone(h: 9, w: 60, delayMs: 80 + i * 40),
                  ],
                ),
              ),
            ]),
        ...
        Row(
          ...
          children: [
            _bone(h: 14, w: 140, delayMs: 120),
            _bone(h: 24, w: 60, r: 20, delayMs: 130),
          ],
        ),
        const SizedBox(height: 12),
        ...
        ...List.generate(3, (i) => Padding(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
                color: bgCard, borderRadius: AppRadius.lg),
            child: Row(children: [
              _bone(h: 36, w: 36, circle: true, delayMs: 80 + i * 60),
              ...
              Expanded(
                child: Column(
                  ...
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    _bone(h: 12, delayMs: 100 + i * 60),
                    const SizedBox(height: 7),
                    _bone(h: 9, w: 160, delayMs: 120 + i * 60),
                  ],
                ),
              ),
              const SizedBox(width: 12),
              _bone(h: 28, w: 68, r: 14, delayMs: 140 + i * 60),
            ]),
          ),
        )),
      ],
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(
              ...
              style: TextStyle(
                  fontSize: 13, color: txtSecond, height: 1.5),
...
class _DashboardAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final topBg    = isDark
        ? const Color(0xFF1A0E4F) : const Color(0xFFEBEEFF);
...
// ── AppBar skeleton while loading ────────────────────────────
class _AppBarSkeleton extends StatelessWidget {
  final bool isDark;
  const _AppBarSkeleton({required this.isDark});

  Widget _bone({double h = 14, double? w, double r = 8, bool circle = false}) =>
      Container(
        height: h,
        width:  circle ? h : w,
        decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _bone(h: 10, w: 80),
            const SizedBox(height: 8),
            _bone(h: 22, w: 130, r: 6),
          ]),
          Row(children: [
            _bone(h: 26, w: 90, r: 20),
            const SizedBox(width: 8),
            _bone(h: 40, w: 40, circle: true),
          ]),
        ],
      ),
    );
...
// ─────────────────────────────────────────────────────────────
//  Universal user avatar widget
//  Shows network image → falls back to initials on error
// ─────────────────────────────────────────────────────────────
      ...
      return CircleAvatar(
        ...
        child: ClipOval(
          child: Image.network(
            ...
            height: radius * 2,
            fit:    BoxFit.cover,
            // On load error: fall back to initial letter
...
class _StatsRow extends StatelessWidget {
  ...
  final Color bgCard;
  const _StatsRow({
    required this.tasksData, required this.skillsData,
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: StatCard(
        icon: Iconsax.task_square, label: 'Tasks Done',
        value: '${tasksData['completed'] ?? 0}',
        color: AppColors.accent,
      )),
      const SizedBox(width: 12),
      Expanded(child: StatCard(
        icon: Iconsax.book, label: 'Skills',
        value: '${skillsData['enrolled'] ?? 0}',
        color: AppColors.gold,
      )),
      const SizedBox(width: 12),
      Expanded(child: StatCard(
        icon: Iconsax.activity, label: 'Active',
...
class _IncomeToolsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tiles = [
      (Icons.auto_awesome_rounded, 'Agentic AI',  'Execute any task',
          AppColors.accent,            '/agent',      'APEX'),
      (Iconsax.flash,              'Workflows',   'AI income plans',
          AppColors.success,           '/workflow',   null),
      (Icons.trending_up_rounded,  'Market Pulse','What pays now',
          const Color(0xFFFF6B35),     '/pulse',      'LIVE'),
      (Icons.emoji_events_rounded, 'Challenges',  '30-day sprints',
          const Color(0xFF6C5CE7),     '/challenges', null),
      (Iconsax.briefcase,          'Client CRM',  'Track deals',
          AppColors.primary,           '/crm',        null),
      (Iconsax.document_text,      'Contracts',   'AI contracts',
          AppColors.gold,              '/contracts',  null),
      (Iconsax.gallery,            'Portfolio',   'Showcase work',
          const Color(0xFFFF3CAC),     '/portfolio',  null),
      (Icons.psychology_rounded,   'Memory',      'Income DNA',
          const Color(0xFF00B894),     '/memory',     null),
    ];
        ...
        ...tasks.asMap().entries.map((e) => Padding(
          ...
          child: TaskPreviewCard(
            ...
            onTap: () => context.go('/tasks'),
              ...
              delay: Duration(milliseconds: 260 + e.key * 70)),
        )),
      ]);
    }
...
class _EarningsHero extends StatelessWidget {
  ...
  const _EarningsHero({
    ...
    required this.currencySymbol, required this.stage,
    required this.stageInfo, required this.isPremium,
    required this.isDark, required this.bgCard,
    required this.txtPrimary, required this.txtSecond,
  });
...
class _DashboardDrawer extends StatelessWidget {
  ...
  const _DashboardDrawer({
    ...
    required this.bgCard, required this.bgSurface,
    required this.txtPrimary, required this.txtSecond,
    required this.txtMuted, required this.divider,
  });
