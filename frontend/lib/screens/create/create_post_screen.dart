...
class _CreatePostScreenState extends State<CreatePostScreen> {
      ...
      if (_videoDur.inMilliseconds > 0) {
        ...
        if (pos.inMilliseconds >= endMs) {
          ...
          _videoCtrl!.seekTo(Duration(
              milliseconds:
                  (_trim.start * _videoDur.inMilliseconds).round()));
  ...
  void _togglePlay() {
    ...
    if (_videoPlaying) {
      ...
      final endMs   = (_trim.end   * _videoDur.inMilliseconds).round();
      if (_videoPos.inMilliseconds < startMs ||
          _videoPos.inMilliseconds >= endMs) {
  ...
  Future<void> _post() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_mediaUrl != null
              ? 'Post submitted for review! 🔍' : 'Post shared! 🚀'),
      ...
      if (mounted) {
        ...
        _showErr('Failed to post. Try again.');
      }
    }
  }
        ...
        SliderTheme(
          ...
          child: RangeSlider(
            ...
            onChanged: (v) {
              ...
              _videoCtrl?.seekTo(Duration(
                  milliseconds:
                      (v.start * _videoDur.inMilliseconds).round()));
            },
          ),
        ),
        Row(children: [
          ...
          const Spacer(),
          Text(_fmtS(_videoDur.inSeconds.toDouble()),
