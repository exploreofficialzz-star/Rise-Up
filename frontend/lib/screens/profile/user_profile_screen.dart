...
class _UserProfileScreenState extends State<UserProfileScreen>
  ...
  Future<void> _sendMessage() async {
    try {
      ...
      if (mounted) _showErrorSnackBar('Could not open message thread');
    }
  }
  ...
  // FIX 2: Renamed local variable from 'num' to 'count' — 'num' shadows the Dart type
  ...
  Widget build(BuildContext context) {
    ...
    // FIX 3: Removed unused 'surface' and 'currencyFormat' variables
    // that were declared but never referenced — prevents analyzer warnings
                    ...
                    actions: [
                      ...
                      IconButton(
                        icon: Icon(Icons.more_vert_rounded,
                            color: text, size: 20),
                        onPressed: () =>
                            _showMoreSheet(isDark, text, sub),
                      ),
                    ],
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    _StatChip(
                                        _fmt(_stats['followers']),
                                        'Followers',
                                        Colors.white),
                                    const SizedBox(width: 20),
                                    _StatChip(
                                        _fmt(_stats['following']),
                                        'Following',
                                        Colors.white),
                                    const Spacer(),
                                    if (earned > 0)
    ...
    for (final match in matches) {
      if (match.start > lastEnd) {
        spans.add(TextSpan(
          ...
          style:
              TextStyle(fontSize: 13, color: textColor, height: 1.5),
        ));
      }
      spans.add(TextSpan(
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _moreOption(Icons.ios_share_rounded, 'Share Profile',
                sub, _shareProfile),
            if (!_isOwnProfile) ...[
              _moreOption(Icons.block_rounded, 'Block User',
                  Colors.red, () {
                Navigator.pop(context);
                _showBlockConfirmation();
              }),
              _moreOption(Icons.flag_rounded, 'Report User',
                  Colors.orange, () {
                Navigator.pop(context);
                _showReportSheet();
              }),
  ...
  void _showBlockConfirmation() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'You won\'t see posts or messages from '
            '${(_profile['full_name']?.toString() ?? 'this user').split(' ').first}.'),
        actions: [
          TextButton(
...
class _TabDelegate extends SliverPersistentHeaderDelegate {
  ...
  Widget build(BuildContext _, double __, bool ___) => Container(
        color: bg,
        child: Column(
            children: [tabBar, Divider(height: 1, color: border)]),
      );
