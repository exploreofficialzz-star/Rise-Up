...
def _post_dict(post: dict, author: dict, like_count: int, is_liked: bool) -> dict:
    ...
    if author:
        ...
        author_id   = str(author.get("id", author_id))
...
async def list_groups(current_user: dict = Depends(get_current_user)):
    ...
    for g in groups:
        gid = str(g["id"])
        count_res = (
            db.table("group_members").select("id", count="exact")
            .eq("group_id", gid).execute()
        )
        member_count = count_res.count or 0
        joined_res = (
            db.table("group_members").select("id")
            .eq("group_id", gid).eq("user_id", user_id).execute()
        )
        out.append(_group_dict(g, member_count, bool(joined_res.data)))
...
async def create_group(
    ...
    res = None
    try:
        res = db.table("groups").insert(full_row).execute()
    except Exception:
        try:
            res = db.table("groups").insert(minimal_row).execute()
        except Exception as e2:
            raise HTTPException(status_code=500, detail=f"Failed to create group: {e2}")
    ...
    db.table("group_members").insert({
        ...
        "user_id": user_id, "is_admin": True, "joined_at": now,
    }).execute()
...
async def get_group(group_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return {"group": _group_dict(group, count_res.count or 0, bool(joined_res.data))}


# ── POST /groups/{group_id}/join ──────────────────────────────────────────────

@router.post("/{group_id}/join")
async def toggle_join(group_id: str, current_user: dict = Depends(get_current_user)):
    user_id = _uid(current_user)
    if not db.table("groups").select("id").eq("id", group_id).execute().data:
        raise HTTPException(status_code=404, detail="Group not found")
...
async def list_posts(group_id: str, current_user: dict = Depends(get_current_user)):
    user_id = _uid(current_user)
    res = (
        db.table("group_posts").select("*")
    ...
    for p in posts:
        ...
        like_res = (
            db.table("group_post_likes").select("id", count="exact")
            .eq("post_id", p["id"]).execute()
        )
        liked_res = (
            db.table("group_post_likes").select("id")
            .eq("post_id", p["id"]).eq("user_id", user_id).execute()
        )
        out.append(_post_dict(p, author, like_res.count or 0, bool(liked_res.data)))
...
async def create_post(
    ...
    author = author_res.data[0] if author_res.data else {}
    return {"post": _post_dict(res.data[0], author, 0, False), "message": "Post created"}
...
async def list_members(group_id: str, current_user: dict = Depends(get_current_user)):
    res = (
        db.table("group_members").select("*")
        .eq("group_id", group_id)
        .order("is_admin", desc=True).limit(100).execute()
    )
