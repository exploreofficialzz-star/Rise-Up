...
async def get_feed(
    ...
    try:
        ...
        for p in posts:
            ...
            p["is_liked"] = any(
                l["user_id"] == user["id"] for l in likes
            )
            p["is_saved"] = any(
                s["user_id"] == user["id"] for s in saves
            )
            p["likes_count"] = len(likes)
            p["comments_count"] = (
                comments_data[0].get("count", 0)
                if comments_data
                else 0
            )
            enriched.append(p)
...
async def get_post(
    ...
    try:
        ...
        p["is_liked"] = any(
            l["user_id"] == user["id"]
            for l in (p.get("post_likes") or [])
        )
        p["is_saved"] = any(
            s["user_id"] == user["id"]
            for s in (p.get("post_saves") or [])
        )
        return {"post": p}
...
async def toggle_like(
    ...
    try:
        ...
        if existing:
            ...
            try:
                db.rpc(
                    "decrement_post_likes", {"pid": post_id}
                ).execute()
            except Exception:
                pass
            ...
            try:
                db.rpc(
                    "increment_post_likes", {"pid": post_id}
                ).execute()
            except Exception:
                pass
            ...
            try:
                ...
                if post and post["user_id"] != user["id"]:
                    ...
                    db.table("notifications").insert({
                        ...
                        "title": "New like",
                        "message": (
                            f"{profile.get('full_name', 'Someone')}"
                            " liked your post"
                        ),
...
async def like_comment(
    ...
    try:
        ...
        if existing:
            db.table("comment_likes").delete().eq(
...
async def share_post(
    ...
    try:
        try:
            db.rpc(
                "increment_post_shares", {"pid": post_id}
            ).execute()
        except Exception:
            pass
    ...
    try:
        ...
        posts_res = (
            ...
            .select(
                "*, "
                "profiles("
                "full_name, stage, is_verified, subscription_tier"
                "), "
                "post_likes(user_id), "
                "post_saves(user_id), "
                "post_comments(count)"
...
async def create_status(
    req: StatusCreate, user: dict = Depends(get_current_user)
):
    user_id = user["id"]
    try:
        active = (
        ...
        if (active.count or 0) >= 15:
            raise HTTPException(
                400, "Maximum 15 active statuses allowed"
            )

        expires = (
            datetime.now(timezone.utc)
            + timedelta(hours=req.duration_hours)
        ).isoformat()
...
async def get_status_feed(
    ...
    try:
        ...
        res = (
            db.table("user_status")
            .select(
                "*, profiles(id, full_name, avatar_url, is_online)"
            )
        ...
        # FIX: Bulk fetch views — eliminates N+1 DB query
        ...
        viewed_ids = {
            v["status_id"] for v in (views_res.data or [])
        }
...
async def view_status(
    status_id: str, user: dict = Depends(get_current_user)
):
    try:
        # Upsert view record
        ...
        # FIX: Atomic increment via RPC (fallback to select+update)
        try:
            db.rpc(
                "increment_status_views", {"sid": status_id}
            ).execute()
        except Exception:
            current = (
                db.table("user_status")
            ...
            if current:
                db.table("user_status").update({
                    "views_count": (
                        current.get("views_count") or 0
                    ) + 1
                }).eq("id", status_id).execute()

        return {"viewed": True}
    except Exception:
        # Always return success — viewing is best-effort
        return {"viewed": True}
...
async def upload_status_media(
    ...
    if ct not in allowed:
        raise HTTPException(
            400,
            "Only images (JPEG/PNG/WebP/GIF) and videos (MP4/MOV) allowed",
        )
    ...
    try:
        ...
        public_url = (
            url if isinstance(url, str) else url.get("publicUrl", "")
        )
