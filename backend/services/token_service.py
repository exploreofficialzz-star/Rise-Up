...
class TokenService:
    ...
    async def get_state(user_id: str, is_premium: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            ad_bonus    = data.get("ad_bonus_tokens", 0)
            ad_watches  = data.get("ad_watches_today", 0)
            total_avail = daily_limit + ad_bonus
            ...
            return {
                ...
                "percent_used":       int(used / total_avail * 100) if total_avail > 0 else 0,
                "is_premium":         is_premium,
                "exhausted":          remaining == 0,
                "can_watch_ad":       (not is_premium) and (ad_watches < MAX_AD_WATCHES_DAY),
            }
        except Exception as e:
            logger.error("TokenService.get_state error: %s", e)
            return {
    ...
    async def deduct(
        ...
        cost        = override_cost or TOOL_TOKEN_COSTS.get(tool_name, 10)
        today       = TokenService._today()
        daily_limit = PREMIUM_DAILY_TOKENS if is_premium else FREE_DAILY_TOKENS
        try:
            sb  = supabase_service.client
            ...
            if remaining < cost and not is_premium:
                return {
                    "allowed":     False,
                    "cost":        cost,
                    "remaining":   remaining,
            ...
            new_used = used + cost
            sb.table("apex_token_quota").upsert(
                {
                    "user_id":          user_id,
                    "quota_date":       today,
            ...
            new_remaining = max(0, total_avail - new_used)
            return {
                "allowed":      True,
                "cost":         cost,
                ...
                "percent_used": int(new_used / total_avail * 100) if total_avail > 0 else 0,
            }
        except Exception as e:
            logger.error("TokenService.deduct error: %s", e)
    ...
    async def grant_ad_tokens(user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            new_bonus   = data.get("ad_bonus_tokens", 0) + AD_TOKENS_PER_WATCH
            ...
            remaining_ads = MAX_AD_WATCHES_DAY - new_watches
            return {
                "granted":          True,
                ...
                "show_subscribe":   remaining_ads == 0,
                "message": (
                    f"🎉 +{AD_TOKENS_PER_WATCH} APEX tokens! "
                    f"{remaining_ads} ad{'s' if remaining_ads != 1 else ''} left today."
                ),
            }
        except Exception as e:
            logger.error("TokenService.grant_ad_tokens error: %s", e)
